...
namespace бхс
{
	public partial class Form1 : Form
	{
		...
		WindowsMediaPlayer femtanylo_player;
		int current_track;
		bool shuffle = false;
		...
		{
			InitializeComponent();
			femtanylo_player = new WindowsMediaPlayer(); 
		}
		...
		private void button2_Click(object sender, EventArgs e)
		{
			...
			if (femtanylo_player.playState == WMPPlayState.wmppsPlaying)
			{
				femtanylo_player.controls.pause();
			}
			else if (femtanylo_player.playState == WMPPlayState.wmppsPaused || femtanylo_player.playState == WMPPlayState.wmppsStopped)
			{
				femtanylo_player.controls.play();
				while (shuffle)
				{
					femtanylo_player.controls.currentPosition = 0;
				}
			}
		}
		...
		{
			femtanylo_player.controls.stop();
		}
		...
		void AddAudioFile()
		{
			openFileDialog1.Title = "Мы украдём все ваши данные, и продадим 3 лицам";
